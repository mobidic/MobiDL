		...
		if (doCrumble) {
			call runCleanUpPanelCaptureTmpDirs.cleanUpPanelCaptureTmpDirs as cleanUpPanelCaptureTmpDirsDoCrumble {
				input:
					...
					BamArray = ["~{dataPath}" + basename(sambambaMarkDup.markedBamIndex), "~{dataPath}" + basename(gatkGatherBQSRReports.gatheredRecalTable), "~{dataPath}" + basename(gatkApplyBQSR.recalBamIndex), "~{dataPath}" + basename(gatkLeftAlignIndels.lAlignedBam), "~{dataPath}" + basename(gatkLeftAlignIndels.lAlignedBamIndex), "~{dataPath}" + basename(samtoolsSort.sortedBam), "~{dataPath}" + basename(finalIndexing.bamIndex), "~{dataPath}" + basename(samtoolsCramConvert.cram),"~{dataPath}" + basename(samtoolsCramIndex.cramIndex)],
