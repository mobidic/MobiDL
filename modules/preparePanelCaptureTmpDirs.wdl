...
task preparePanelCaptureTmpDirs {
	...
	command <<<
		...
		if [ ! -d "~{OutDir}~{SampleID}/~{WorkflowType}/FastpDir" ];then \
			mkdir -p "~{OutDir}~{SampleID}/~{WorkflowType}/FastpDir"; \
		fi
		...
		if [ ! -d "~{OutDir}~{SampleID}/~{WorkflowType}/splitted_intervals" ];then \
			mkdir -p "~{OutDir}~{SampleID}/~{WorkflowType}/splitted_intervals"; \
		fi
		if [ ! -d "~{OutDir}~{SampleID}/~{WorkflowType}/recal_tables" ];then \
			mkdir -p "~{OutDir}~{SampleID}/~{WorkflowType}/recal_tables"; \
		fi
		...
		echo "Workflow: MobiDL ~{WorkflowType} v1.3.2" >> "~{OutDir}~{SampleID}/~{WorkflowType}/~{SampleID}.versions.txt"
