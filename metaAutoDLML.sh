...
modifyJson() {
    ...
    MOBIDL_JSON_TEMPLATE="${MOBIDL_JSON_DIR}${WDL}_inputs_minibwa_${GENOME}.json"
    # if [ "${GENOME}" != "hg19" ];then
...
gatherJsonsAndLaunch() {
    ...
    for aList in "${AUTODL_DIR}${RUN}"/samplesInfos_${metaWDL}.* ; do
        ...
        if [ ! -e "${MOBIDL_JSON_TEMPLATE}" ];then
            ...
            grep -vE "\{|\.fastqR1|\.fastqR2|\.suffix1|\.suffix2|\.outDir|\.intervalBedFile|\.sampleID|\.geneFile" "${MOBIDL_JSON_TEMPLATE}" |    sed "s/${WDL}\./${metaWDL}\./" >> "${JSON}"
            debug "$(cat ${JSON})"
            info "Launching:"
...
workflowPostTreatment() {
    ...
    if [ "${DRY_RUN}" = true ];then
        ...
        parallel -j 8 /usr/bin/srun -N1 -c1 -pprod -JautoDL_rsync_sample "${RSYNC}" -aqz --no-g --no-times --chmod=ugo=rwX {} "${OUTPUT_PATH}${RUN}/MobiDL/${DATE}/" ::: "${TMP_OUTPUT_DIR2}"/*
        if [ $? -eq 0 ];then
...
prepareGatkCnv() {
    ...
    rsync "${GATK_YAML}" "${1}gatk_cnv.yaml"
    PROJECT_PATH_SED=${1////\\/}
                ...
                if [[ -r ${SAMPLESHEET_PATH} ]];then
                    ...
                    if [ -n "${MANIFEST}" ];then
                        ...
                        if [ -n "${MANIFEST}" ] &&  [ -n "${WDL}" ] && [ -n "${BED}" ];then
                            ...
                            # create a sample arry to launch the nf-core rnaseq wrapper
                            declare -A RNA_SAMPLE_ARRAY
                            for SAMPLE in ${!SAMPLES[@]};do
                                if [[ ${MULTIPLE} != '' ]];then
                                    ...
                                    if [[ ! -f "${ROI_DIR}${BED}" || ! -f "${WDL_PATH}${NEW_WDL}.wdl" ]];then
                                        # RNA?
                                        debug "PROVIDER:${PROVIDER}-NEW_WDL:${NEW_WDL}-BED:${BED}"
                                        # if [ "${PROVIDER}" = "ELEMENT" ] && [ "${NEW_WDL}" = "rnaseq" ] && [ "${BED}" = "nobed" ];then
                                        if [ "${NEW_WDL}" = "rnaseq" ] && [ "${BED}" = "nobed" ];then
                                            ...
                                            # fi
                                            if [[ ! -v ${RNA_SAMPLE_ARRAY["PATH"]} ]];then
                                                RNA_SAMPLE_ARRAY["PATH"]=$(echo "${SAMPLES[${SAMPLE}]}" | cut -d ';' -f 3)
                                                # debug "RNA_SAMPLE_ARRAY["PATH"]:${RNA_SAMPLE_ARRAY["PATH"]}"
                                            fi
                                            # key is group, value is sample list
                                            RNA_GROUp="Illumina"
                                            if [ "${PROVIDER}" = "ELEMENT" ];then
                                                RNA_GROUP=$(echo "${SAMPLES[${SAMPLE}]}" | cut -d ';' -f 4)
                                            fi
                                            if [[ -v RNA_SAMPLE_ARRAY["${RNA_GROUP}"] ]];then
                                                RNA_SAMPLE_ARRAY[${RNA_GROUP}]="${RNA_SAMPLE_ARRAY[${RNA_GROUP}]},${SAMPLE}"
                                                # debug "RNA_GROUP: 2-${RNA_GROUP}-${RNA_SAMPLE_ARRAY[${RNA_GROUP}]}"
                                            else
                                                debug "RNA_GROUP: 1-${RNA_GROUP}"
                                                RNA_SAMPLE_ARRAY[${RNA_GROUP}]="${SAMPLE}"
                                            fi
                                        elif [ "${DRY_RUN}" = false ];then
                            ...
                            debug "RNA_SAMPLE_ARRAY FINAL: ${#RNA_SAMPLE_ARRAY[@]}"
                            ################ RNA workflow - ELEMENTBIO ONLY
                            # run nfcore rnaseq via its home-made wrapper
                            if [[ ${#RNA_SAMPLE_ARRAY[@]} != 0 ]];then
                                # get CWD
                                MOBIDL_LAUNCH_PATH="${PWD}"
                                for RNA_GROUP in "${!RNA_SAMPLE_ARRAY[@]}";do
                                    if [ "${RNA_GROUP}" = "PATH" ];then 
                                        continue
                                    fi
                                    # get fastq dir
                                    # FASTQ_DIR=$(echo "${RNA_SAMPLE_ARRAY[${RNA_SAMPLE}]}" | cut -d ';' -f 3)
                                    # RNA_FASTQ_DIR=${FASTQ_DIR}
                                    # RNA_SAMPLE is indeed the group
                                    RNA_FASTQ_DIR=${RNA_SAMPLE_ARRAY["PATH"]}${RNA_GROUP}
                                    if [ "${DRY_RUN}" = false ];then
                                        # launch here
                                        info "launching nf-core rnaseq: cd ${TMP_OUTPUT_DIR} && ${NFCORE_RNASEQ_WRAPPER} ${RNA_FASTQ_DIR} ${RNA_SAMPLE_ARRAY[${RNA_GROUP}]}"
                                        cd "${TMP_OUTPUT_DIR}" && nohup "${NFCORE_RNASEQ_WRAPPER}" "${RNA_FASTQ_DIR}" "${RNA_SAMPLE_ARRAY[${RNA_GROUP}]}" > ${TMP_OUTPUT_DIR}/${RUN}_${RNA_GROUP}_rnaseq.log 2>&1 &
                                        # return to PWD
                                        cd "${MOBIDL_LAUNCH_PATH}"
                                    else
                                        debug "MOBIDL_LAUNCH_PATH: ${MOBIDL_LAUNCH_PATH}"
                                        debug "launching nf-core rnaseq: cd ${TMP_OUTPUT_DIR} && ${NFCORE_RNASEQ_WRAPPER} ${RNA_FASTQ_DIR} ${RNA_SAMPLE_ARRAY[${RNA_GROUP}]}"
                                    fi
                                done
                            fi
                            ################
                            ...
                            for SAMPLE in ${!SAMPLES[@]};do
                                ...
                                TREATED=1
                                # gatk_cnv specific feature: create a folder with symbolic links to the alignment files
                        ...
                        if [ "${TREATED}" -eq 1 ];then
                            # MobiCNV && multiqc
                            # no VCF anymore for mobicnv: -v ${OUTPUT_PATH}${RUN}/MobiDL/${DATE}/MobiCNVvcfs/
                            if [ -n "${MULTIPLE}" ];then
                                ...
                                do
                                    ...
                                    if [[ "${LIBRARY}" != "${EXOME_GROUP}" ]];then
                                        ...
                                        if [ ${NUMBER_OF_SAMPLE} -gt 2 ];then
                                            ...
                                            info "/usr/bin/srun -N1 -c1 -pprod -JautoDL_mobicnv ${PYTHON} ${MOBICNV} -i ${MOBICNVTSV_PATH} -t tsv  -o {PROJECT_PATH}${RUN}_${LIBRARY}_MobiCNV.xlsx"
                                        else
                                            info "Not enough samples to launch MobiCNV for Library ${LIBRARY} (${NUMBER_OF_SAMPLE} samples)"
                                        fi
                                    fi
